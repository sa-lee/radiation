...
plot of the LEC and MEC celllines. There is clear differential expression between treatment and celltype, but also confounding caused by replicate number.
...
```{r hmec}
hmec_lec_top <- top_tables %>% 
  mutate(updown = sign(logFC)) %>% 
  filter(grepl("HMEC|LEC", contrast), 
         adj.P.Val < 0.05) 
  
hmec_lec_top_de <- hmec_lec_top %>% 
  count()
hmec_updown <- hmec_lec_top %>% 
  group_by(contrast, updown) %>% 
  count()


```
Due to the large numbers of contrasts we focus on two cellines and their
response to radiotherapy treatment: MEC and LEC. For 
microvascular blood endothelial cells, there were `r hmec_lec_top_de %>% filter(contrast == "HMEC_10GY-HMEC_0GY") %>% pull(n)` 
genes differentially upregulated between the full dosage (10Gy) radiotherapy 
and control treatments.

While for lymphatic endothelial cells there were 
`r hmec_lec_top_de %>% filter(contrast == "LEC_10GY-LEC_0GY") %>% pull(n)`
differentialy expressed genes between full dosage (10Gy) radiotherapy
and contorl treatments. Of these 
`r hmec_updown %>% filter(contrast == "LEC_10GY-LEC_0GY", updown == -1) %>% pull(n)`
genes were downregulated and 
`r hmec_updown %>% filter(contrast == "LEC_10GY-LEC_0GY", updown == 1) %>% pull(n)`
were upregulated. There were 
`r hmec_lec_top_de %>% filter(contrast == "LEC_2GYx5-LEC_0GY") %>% pull(n)`
genes differentially expressed between LEC cells under fractionated dosage
of radiotherapy and control treatment. Of these 
`r hmec_updown %>% filter(contrast == "LEC_2GYx5-LEC_0GY", updown == -1) %>% pull(n)`
were downregulated and 
`r hmec_updown %>% filter(contrast == "LEC_2GYx5-LEC_0GY", updown == 1) %>% pull(n)`
were upregulated.


Between LEC and MEC cells under no treatment there were
`r hmec_lec_top_de %>% filter(contrast == "LEC_0GY-HMEC_0GY") %>% pull(n)`
differentially expressed genes, 
`r hmec_updown %>% filter(contrast == "LEC_0GY-HMEC_0GY", updown == -1) %>% pull(n)`
downregulated and 
`r hmec_updown %>% filter(contrast == "LEC_0GY-HMEC_0GY", updown == 1) %>% pull(n)`
, respectively. Between celllines under full radiotherapy treatment, there
were `r hmec_lec_top_de %>% filter(contrast == "LEC_10GY-HMEC_10GY") %>% pull(n)`
differentially expressed genes, 
`r hmec_updown %>% filter(contrast == "LEC_10GY-HMEC_10GY", updown == -1) %>% pull(n)`
downregulated and 
`r hmec_updown %>% filter(contrast == "LEC_10GY-HMEC_10GY", updown == 1) %>% pull(n)`
