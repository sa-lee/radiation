...
counts_mat <- edgeR::cpm(counts_mat)[gene_filter, ]
...
In this experiment, the effect radiotherapy has on gene expression was measured 
...
the _featureCounts_ procedure using default settings. Transcripts were anntotated
...
split accross three sequencing lanes, and its cpm values were merged together.
Genes were retained for further analysis if they had a baseline expression
level of 0.5 cpm in at least 2 samples. 
...
variation in the data (figure x). There is a clear seperation between the 
different cell-lines and the replicate number from the MDS plots. We model
the heteroscedasticty using the _voomWithQualityWeights_ procedure using 
the cell-line specific radiotherapy treatment as a main effect. Given the clear 
difference between replicates (related to when the experiment was carried out), 
we include a random effect for replicate in the model . 
...
ggplot(mean_variance_df, aes(x = mean_signal, y = variance_signal)) +
  geom_point() +
...
This RNA-seq data set was generated by  from the 
