...
readr::write_excel_csv(samples, "geo_submission/samples.txt")

# proccessed counts
processed_counts <- readr::read_rds("geo_submission/processed_counts.rda")
...
for (i in seq_len(ncol(processed_counts))) {
  ...
  gene_counts <- data_frame(entrez_gene_id = processed_counts$genes$GeneID,
                            gene_length = processed_counts$genes$Length,
                            read_count = sample_counts)
  readr::write_tsv(gene_counts, 
            path = paste0("geo_submission/summarised_counts/",
                          group, "_", replicate, ".txt"))
}
